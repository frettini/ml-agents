...
class ForwardKinematics:
    def __init__(self, edges):
        ...
        self.world = False#args.fk_world # default false
        self.pos_repr = '3d'#args.pos_repr
        self.quater = True#args.rotation == 'quaternion' # worst bool name :(

    def forward_from_raw(self, raw, offset, world=None, quater=None):

        # print("[FK] raw input: {}, offset input: {}".format(raw.shape, offset.shape))
    ...
    def forward(self, rotation: torch.Tensor, position: torch.Tensor, offset: torch.Tensor, order='xyz', quater=False, world=True):
        ...
        # print("[FK] rotation input: {}, position input: {}, offset input: {}".format(rotation.shape, position.shape, offset.shape))
