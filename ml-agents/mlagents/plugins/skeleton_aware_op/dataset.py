...
class TemporalMotionData(torch.utils.data.Dataset):
    ...
    def __init__(self, input_path:str, recalculate_mean_var:bool = False, normalize_data:bool = False, device:str='cpu'):
    ...
    def __getitem__(self, index):
        ...
        if self.normalize_data is True:
            # print(motion.shape)
            # print(self.mean_var.shape)
            # print(self.mean_var[np.newaxis, 0,:,:].shape)
...
class SkeletonInfo:
    def __init__(self, input_path):
        ...
        self.offsets = torch.tensor(anim.offsets[:,:]).float()
...
# UTILS
# -----------------------------------------------
def get_skeleton_info(input_path : str):
    """
    Returns information about the skeleton topology using an example motion file 
    specified by file_path. 
    Args:
    ...
    # generate edge list
    num_joints = len(anim.parents)
    edges = build_edges(anim.parents)
    ...
    # and proportions. This means that there are an equivalent number of chains.
    # We extract the joint indices of each chain here, which will be used for some losses.
    chain_indices = build_chain_list(edges)

    # generate skeleton offsets, shape [J,3]       
    offsets = torch.tensor(anim.offsets[:,:]).float()

    return edges, offsets, ee_id, chain_indices, anim.parents, frametime
