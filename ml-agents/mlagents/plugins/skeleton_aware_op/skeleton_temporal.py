...
# Skeleton Convolution
# One kernel per joint
# need to build a neighbour_list according to the hierarchal structure of the skeleton
class SkeletonConv(torch.nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
                 bias=True, padding_mode='reflection', add_offset=False, in_offset_channel=0):
        self.in_channels_per_joint = in_channels // joint_num
        self.out_channels_per_joint = out_channels // joint_num

        # number of channels must be proportial to the number of joints?
    ...
    def forward(self, input):
        ...
        # conv 1D
        # print(input.shape, self._padding_repeated_twice, self.padding_mode)
        res = torch.nn.functional.conv1d(torch.nn.functional.pad(input, self._padding_repeated_twice, mode=self.padding_mode),
