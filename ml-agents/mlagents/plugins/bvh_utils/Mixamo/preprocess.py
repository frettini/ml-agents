...
import numpy as np
import copy
from mlagents.plugins.bvh_utils.bvh_parser import BVH_file
from mlagents.plugins.bvh_utils.motion_dataset import MotionData
from option_parser import get_args, try_mkdir


def collect_bvh(data_path, character, files, subsample=False):
    ...
    for i, motion in enumerate(files):
        ...
        if subsample is True:
            file.anim.rotations = file.anim.rotations[::2,:]
            file.anim.positions = file.anim.positions[::2,:,:]
            file.frametime *= 2
        modified_motion = str.replace(motion, '.bvh', '_m.bvh')
        
        file.write(data_path + character + '/' + modified_motion)
...
def copy_std_bvh(data_path, character, files):
    ...
    """
    cmd = 'copy \"{}\" .\datasets\Ortiz_extend\std_bvhs\{}.bvh'.format(data_path + character + '\\' + files[0], character)
    os.system(cmd)


if __name__ == '__main__':
    prefix = './datasets/Ortiz_extend/'
    ...
    for character in characters:
        ...
        collect_bvh(prefix, character, files, subsample=False)
        copy_std_bvh(prefix, character, files)
        write_statistics(character, './datasets/Ortiz_extend/mean_var/')
